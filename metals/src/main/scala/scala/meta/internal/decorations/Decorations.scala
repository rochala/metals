...
abstract class Decorations[T](
  ...
  protected def refreshAction(doc: AbsolutePath): Future[Unit]

  protected def onChange(path: AbsolutePath, textDoc: TextDocument)
  ...
  override def onChange(
    ...
    for {
      ...
      onChange(path, textDoc)
    }
  }

  def refresh(): Future[Unit] = {
    focusedDocument() match {
      case Some(doc) => refreshAction(doc)
      case None => Future.unit
    }
  }

  protected def toDecoration(
      lspRange: l.Range,
  ...
  protected def syntheticDecorations(
      path: AbsolutePath
  ): Seq[T] = {
    ...
    currentDocument(path) match {
      case Some(doc) if isFocusedDocument =>
        scribe.info("Decorations: computing decorations")
  ...
  protected def areSyntheticsEnabled: Boolean = {
    ...
    val typeDecorations = for {
      ...
      signature.signature match {
        case m: MethodSignature =>
          ...
          Some(toDecoration(lspRange, decoration))
